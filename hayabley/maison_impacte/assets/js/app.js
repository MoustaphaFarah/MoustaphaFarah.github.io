...
var route1 = L.geoJson(null, {
  ...
  onEachFeature: function (feature, layer) {
    if (feature.properties) {
      var content = "<table class='table table-striped table-bordered table-condensed'>" + "<tr><th>Nom</th><td>" + feature.properties.Name + "<tr><th>Longueur</th><td>" + feature.properties.Longueur + 'Kilometre' + "<table>";
      layer.on({
        click: function (e) {
    ...
    layer.on({
      ...
      mouseout: function (e) {
        route1.resetStyle(e.target);
      }
    });
  }
});
$.getJSON("data/route1.geojson", function (data) {
  route1.addData(data);
});
...
var route2 = L.geoJson(null, {
  ...
  onEachFeature: function (feature, layer) {
    if (feature.properties) {
      var content = "<table class='table table-striped table-bordered table-condensed'>" + "<tr><th>Nom</th><td>" + feature.properties.Name + "<tr><th>Longueur</th><td>" + feature.properties.Longueur + 'Metre' + "<table>";
      layer.on({
        click: function (e) {
          $("#feature-title").html(feature.properties.NAME);
          $("#feature-info").html(content);
    ...
    layer.on({
      ...
      mouseout: function (e) {
        route2.resetStyle(e.target);
      }
    });
  }
});
$.getJSON("data/route2.geojson", function (data) {
  route2.addData(data);
});
...
var zone4 = L.geoJson(null, {
  pointToLayer: function (feature, latlng) {
    return L.marker(latlng, {
      ...
      title: feature.properties.nomchef,
      riseOnHover: true
    });
  },
  onEachFeature: function (feature, layer) {
    if (feature.properties) {
       var content = "<table class='table table-striped table-bordered table-condensed'>" + "<tr><th>NOM</th><td>" + feature.properties.nomchef + "</td></tr>" + "<tr><th>SEX</th><td>" + feature.properties.sexchef + "</td></tr>" +  "<tr><th>Nationalté</th><td>" + feature.properties.nationalit + "</td></tr>" + "</td></tr>" + "<tr><th>Telephone</th><td>" + feature.properties.telephon1 + "</td></tr>" + "</td></tr>" + "<tr><th>Type de toilette</th><td>" + feature.properties.typetoilet + "</td></tr>" + "</td></tr>" + "<tr><th>Statut du logement</th><td>" + feature.properties.logement1 + "</td></tr>" + "</td></tr>"   +  '<img src="'+ feature.properties.photo +'" style="width:300px;height:300px;">'  +"<table>";
      layer.on({
        click: function (e) {
          $("#feature-title").html(feature.properties.nomchef);
          $("#feature-info").html(content);
          $("#featureModal").modal("show");
          highlight.clearLayers().addLayer(L.circleMarker([feature.geometry.coordinates[1], feature.geometry.coordinates[0]], highlightStyle));
        }
      ...
      $("#feature-list tbody").append('<tr class="feature-row" id="' + L.stamp(layer) + '" lat="' + layer.getLatLng().lat + '" lng="' + layer.getLatLng().lng + '"><td style="vertical-align: middle;"><img width="16" height="18" src="assets/img/map1.png"></td><td class="feature-name">' + layer.feature.properties.nomchef+ '</td><td style="vertical-align: middle;"><i class="fa fa-chevron-right pull-right"></i></td></tr>');
      zone4Search.push({
         name: layer.feature.properties.nomchef,
        address: layer.feature.properties.localite,
        source: "zone4",
        id: L.stamp(layer),
        lat: layer.feature.geometry.coordinates[1],
...
$.getJSON("data/maison_impacte.geojson", function (data) {
  zone4.addData(data);
});
...
var zone4BH = new Bloodhound({
  ...
  $("#searchbox").typeahead({
    ...
    templates: {
      header: "<h4 class='typeahead-header'><img src='assets/img/map1.png' width='35' height='30'>&nbsp; Ménage impacté</h4>",
      suggestion: Handlebars.compile(["{{name}}<br>&nbsp;<small>{{address}}</small>"].join(""))
    }
  },{
    name: "GeoNames",
