var map, featureList, boroughSearch = [], poteauSearch = [], compteurSearch = [], polycliniqueSearch  = [], priveSearch  = [], confiSearch  = [];
...
function syncSidebar() {
 ...
 /* Loop through polyclinique layer and add only features which are in the map bounds */
  polyclinique.eachLayer(function (layer) {
    if (map.hasLayer(polycliniqueLayer)) {
      if (map.getBounds().contains(layer.getLatLng())) {
        $("#feature-list tbody").append('<tr class="feature-row" id="' + L.stamp(layer) + '" lat="' + layer.getLatLng().lat + '" lng="' + layer.getLatLng().lng + '"><td style="vertical-align: middle;"><img width="16" height="18" src="assets/img/pol.png"></td><td class="feature-name">' + layer.feature.properties.Etablissem + '</td><td style="vertical-align: middle;"><i class="fa fa-chevron-right pull-right"></i></td></tr>');
      }
    }
  });
   /* Loop through privé layer and add only features which are in the map bounds */
  prive.eachLayer(function (layer) {
    if (map.hasLayer(priveLayer)) {
      if (map.getBounds().contains(layer.getLatLng())) {
        $("#feature-list tbody").append('<tr class="feature-row" id="' + L.stamp(layer) + '" lat="' + layer.getLatLng().lat + '" lng="' + layer.getLatLng().lng + '"><td style="vertical-align: middle;"><img width="16" height="18" src="assets/img/prive.png"></td><td class="feature-name">' + layer.feature.properties.Etablissem + '</td><td style="vertical-align: middle;"><i class="fa fa-chevron-right pull-right"></i></td></tr>');
      }
    }
  });
...
$.getJSON("data/cmh.geojson", function (data) {
  compteur.addData(data);
});

/* Empty layer placeholder to add to layer control for listening when to add/remove polyclinique to markerClusters layer */
var polycliniqueLayer = L.geoJson(null);
var polyclinique = L.geoJson(null, {
  pointToLayer: function (feature, latlng) {
    return L.marker(latlng, {
      icon: L.icon({
        iconUrl: "assets/img/pol.png",
        iconSize: [24, 28],
        iconAnchor: [12, 28],
        popupAnchor: [0, -25]
      }),
...
$.getJSON("data/polyclinique.geojson", function (data) {
  polyclinique.addData(data);
  map.addLayer(polycliniqueLayer);
});

/* Empty layer placeholder to add to layer control for listening when to add/remove polyclinique to markerClusters layer */
var priveLayer = L.geoJson(null);
var prive = L.geoJson(null, {
  pointToLayer: function (feature, latlng) {
    return L.marker(latlng, {
      icon: L.icon({
        iconUrl: "assets/img/prive.png",
        iconSize: [24, 28],
        iconAnchor: [12, 28],
        popupAnchor: [0, -25]
      }),
  ...
  onEachFeature: function (feature, layer) {
    if (feature.properties) {
      var content = "<table class='table table-striped table-bordered table-condensed'>" + "<tr><th>Nom</th><td>" + feature.properties.Etablissem + "</td></tr>" + "<tr><th>Région</th><td>" + feature.properties.Region + "</td></tr>" + "<tr><th>Statut</th><td>" + feature.properties.Statut + "</td></tr>" + "</td></tr>"  +"<table>";
      layer.on({
        click: function (e) {
          $("#feature-title").html(feature.properties.Etablissem);
          $("#feature-info").html(content);
...
/* Empty layer placeholder to add to layer control for listening when to add/remove confi to markerClusters layer */
var confiLayer = L.geoJson(null);
var confi = L.geoJson(null, {
  pointToLayer: function (feature, latlng) {
    return L.marker(latlng, {
      icon: L.icon({
        iconUrl: "assets/img/confi.png",
        iconSize: [24, 28],
        iconAnchor: [12, 28],
        popupAnchor: [0, -25]
      }),
  ...
  onEachFeature: function (feature, layer) {
    if (feature.properties) {
      var content = "<table class='table table-striped table-bordered table-condensed'>" + "<tr><th>Nom</th><td>" + feature.properties.Nom + "</td></tr>"  + "</td></tr>" + "</td></tr>"  +"<table>";
      layer.on({
        click: function (e) {
...
$.getJSON("data/confi.geojson", function (data) {
  confi.addData(data);
  map.addLayer(confiLayer);
});
...
map.on("overlayadd", function(e) {
  ...
  if (e.layer === compteurLayer) {
   ...
   if (e.layer === polycliniqueLayer) {
    markerClusters.addLayer(polyclinique);
    syncSidebar();
  }

   if (e.layer === priveLayer) {
    markerClusters.addLayer(prive);
    syncSidebar();
  }
   if (e.layer === confiLayer) {
    markerClusters.addLayer(confi);
    syncSidebar();
  }
...
map.on("overlayremove", function(e) {
  ...
  if (e.layer === polycliniqueLayer) {
    markerClusters.removeLayer(polyclinique);
    syncSidebar();
  }
  if (e.layer === priveLayer) {
    markerClusters.removeLayer(prive);
    syncSidebar();
  }
  if (e.layer === confiLayer) {
    markerClusters.removeLayer(confi);
    syncSidebar();
  }
});
...
attributionControl.onAdd = function (map) {
  ...
  div.innerHTML = "<span class='hidden-xs'>Développer par DJIB CARTO | </span>";
  return div;
};
...
var groupedOverlays = {
  "  Points d’intérêt ": {
    "<img src='assets/img/csc.png' width='24' height='28'>&nbsp;Centre de Soin Communautaire": poteauLayer,
    "<img src='assets/img/cmh.png' width='24' height='28'>&nbsp;Centre Médical Hospitalier": compteurLayer,
    "<img src='assets/img/pol.png' width='24' height='28'>&nbsp;Polyclinique": polycliniqueLayer,
    "<img src='assets/img/prive.png' width='24' height='28'>&nbsp;Clinique privé": priveLayer,
...
$(document).one("ajaxStop", function () {
  ...
  var polycliniqueBH = new Bloodhound({
    name: "polyclinique",
    datumTokenizer: function (d) {
      return Bloodhound.tokenizers.whitespace(d.name);
    },
    queryTokenizer: Bloodhound.tokenizers.whitespace,
    local: polycliniqueSearch,
    limit: 10
  });
 var priveBH = new Bloodhound({
    name: "prive",
    datumTokenizer: function (d) {
      return Bloodhound.tokenizers.whitespace(d.name);
    },
  ...
  $("#searchbox").typeahead({
    ...
    templates: {
      header: "<h4 class='typeahead-header'><img src='csc.png' width='25' height='25'>&nbsp;Centre de soin communautaire</h4>",
    ...
    name: "polyclinique",
    displayKey: "name",
    source: polycliniqueBH.ttAdapter(),
    templates: {
      header: "<h4 class='typeahead-header'><img src='pol.png' width='35' height='30'>&nbsp;polyclinique</h4>",
      suggestion: Handlebars.compile(["{{name}}<br>&nbsp;<small>{{address}}</small>"].join(""))
    }
  },{
    name: "prive",
    displayKey: "name",
    source: priveBH.ttAdapter(),
    templates: {
      header: "<h4 class='typeahead-header'><img src='prive.png' width='35' height='30'>&nbsp;Cabinet privé</h4>",
      suggestion: Handlebars.compile(["{{name}}<br>&nbsp;<small>{{address}}</small>"].join(""))
    }
  },{
    name: "confi",
    ...
    source: priveBH.ttAdapter(),
    templates: {
      header: "<h4 class='typeahead-header'><img src='prive.png' width='35' height='30'>&nbsp;Site de confinement</h4>",
      suggestion: Handlebars.compile(["{{name}}<br>&nbsp;<small>{{address}}</small>"].join(""))
    }
  },{
  ...
  if (datum.source === "polyclinique") {
    ...
    if (datum.source === "prive") {
      ...
      if (map._layers[datum.id]) {
        ...
        if (datum.source === "confi") {
      if (!map.hasLayer(confiLayer)) {
        map.addLayer(confiLayer);
      }
      map.setView([datum.lat, datum.lng], 17);
      if (map._layers[datum.id]) {
